...
fn it_opens_an_existing_netcdf_file() {
    let result = NetCdfDocumentReader::open_file(NETCDF_ICON_GRID_TEST_FILE);
    assert!(result.is_ok());
...
fn it_returns_an_error_if_the_file_doesnt_exist() {
    let result = NetCdfDocumentReader::open_file(NOT_FOUND_TEST_FILE);
...
fn it_returns_an_error_if_the_file_isnt_in_grib2_format() {
    let result = NetCdfDocumentReader::open_file(NON_NETCDF_TEST_FILE);
