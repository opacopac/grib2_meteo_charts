...
impl FileToGridConverter {
    pub fn read_single_doc_from_file_or_url(file_or_url: &str) -> Result<Grib2Document, Grib2Error> {
        if Self::is_url(file_or_url) {
            ...
            let file = File::open(file_or_url)?;
    ...
    pub fn read_rectangular_grid_from_file(
        ...
        let doc = Self::read_single_doc_from_file_or_url(&file_url)?;
    ...
    fn is_url(file_or_url: &str) -> bool {
        file_or_url.starts_with("http://") || file_or_url.starts_with("https://")
    }
}
