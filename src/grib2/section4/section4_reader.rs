...
impl Section4Reader {
    pub fn read(reader: &mut impl Read) -> Result<Section4, Grib2Error> {
        ...
        let product_definition_template =
            Section4Reader::read_product_definition_template(reader, tpl_number, length - 9)?;
    ...
    fn read_product_definition_template(
        reader: &mut impl Read,
        tpl_number: u16,
        tpl_length: u32,
    ) -> Result<ProductDefinitionTemplate, Grib2Error> {
        let prod_def_tpl = match tpl_number {
            ...
            _ => {
                ...
                ProductDefinitionTemplate::Unknown(tpl_number)
            }
        };
...
mod tests {
    ...
    fn it_correctly_parses_section4_with_a_tpl_4_0() {
        let mut reader = BufReader::new(Cursor::new([
            0x00, 0x00, 0x00, 0x22, 0x04, 0x00, 0x00, 0x00, 0x00, 0x06, 0xC7, 0x02, 0x00, 0x0B,
        ...
        match section4.product_definition_template {
            ...
            _ => panic!(
                "wrong product definition template {:?}",
                section4.product_definition_template
            ),
        };
    ...
    #[test]
    fn it_correctly_parses_section4_with_a_tpl_4_1() {
        let mut reader = BufReader::new(Cursor::new([
            0x00, 0x00, 0x00, 0x25, 0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x04, 0x00, 0x8D,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
            0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC0, 0x00, 0x0B,
        ]));
        ...
        let section4 = result.unwrap();
        assert_eq!(37, section4.length);
        assert_eq!(4, section4.section_number);
        assert_eq!(0, section4.coordinate_values.len());

        match section4.product_definition_template {
            ProductDefinitionTemplate::Template4_1(_tpl) => {}
            _ => panic!(
                "wrong product definition template {:?}",
                section4.product_definition_template
            ),
    ...
    fn it_correctly_parses_section4_with_a_tpl_4_8() {
        let mut reader = BufReader::new(Cursor::new([
            0x00, 0x00, 0x00, 0x3A, 0x04, 0x00, 0x00, 0x00, 0x08, 0x01, 0x34, 0x02, 0x00, 0x0B,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
            0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x07, 0xE6, 0x04, 0x19, 0x01, 0x00, 0x00, 0x01,
            0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x00, 0x00, 0x00, 0x00, 0x3C, 0xFF, 0x00, 0x00,
            0x00, 0x00,
        ]));
    ...
    fn it_correctly_parses_section4_with_coordinate_values_after_tpl() {
        let mut reader = BufReader::new(Cursor::new([
            0x00, 0x00, 0x00, 0x3A, 0x04, 0x00, 0x06, 0x00, 0x00, 0x03, 0x06, 0x02, 0x00, 0x0B,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x96, 0x00, 0x00, 0x00, 0x00, 0x01,
            0x65, 0x00, 0x00, 0x00, 0x00, 0x00, 0x42, 0x84, 0x00, 0x00, 0x40, 0x80, 0x00, 0x00,
            0xBD, 0x5C, 0xDE, 0xAE, 0x09, 0x7E, 0xDC, 0xB9, 0x01, 0x3D, 0xC2, 0x85, 0xE5, 0x5B,
            0x2D, 0x80,
        ]));
