...
use crate::grid::unstructured_value_grid2::UnstructuredValueGrid2;
use std::ops::{Add, Mul};
...
impl UnstructuredGridConverter {
    pub fn create<T: GridValueType + Mul<f32, Output = T> + Add<Output = T> + std::iter::Sum>(
    ...
    ) -> Result<UnstructuredValueGrid2<T>, Grib2Error> {
        ...
        let mut grid = UnstructuredGrid2::new(dimensions, lat_lon_extent, coordinates);
        grid.calc_coord_dist_lookup_map(max_coord_dist_deg);

        let value_grid = UnstructuredValueGrid2::new(unstructured_values, missing_value, grid);
